...
def load_lookup_tables():
    ...
    for sig, ans in zip(train["osig"], train["answer"]):
        osig[sig].append(ans)
    osig_letter = {s: Counter(v).most_common(1)[0][0] for s, v in osig.items()}

    core = defaultdict(list)
    for c, atext in zip(train["core"], train["answer_text"]):
        core[c].append(atext)
    core_answer = {c: Counter(v).most_common(1)[0][0] for c, v in core.items()}

    freq_order = train["answer"].value_counts().index.tolist()
    return known_texts, osig_letter, core_answer, freq_order
...
def load_cross_encoder():
    try:
        ...
        return CrossEncoder("cross-encoder/nli-deberta-v3-small", device="cpu")
    except Exception:
        return None


KNOWN_ANSWER_TEXTS, osig_letter, core_answer, FREQ_ORDER = load_lookup_tables()
...
def length_freq_rank(row):
    scores = {o: len(str(row[o])) + (5 - FREQ_ORDER.index(o)) * 0.5 for o in OPTIONS}
    return sorted(OPTIONS, key=lambda o: -scores[o])
...
def cross_encoder_rank(prompt, row):
    ...
    if ce is None:
        return None
    pairs = [(prompt, str(row[o])) for o in OPTIONS]
    logits = ce.predict(pairs, show_progress_bar=False)
    entail = logits[:, 1] if getattr(logits, "ndim", 1) == 2 else logits
    return [OPTIONS[j] for j in np.argsort(entail)[::-1]]


def predict(prompt, opts):
    row = {"prompt": prompt, "core": normalize_core(prompt)}
    for o, v in zip(OPTIONS, opts):
        row[o] = v

    la = lookup_letter(row)
    hedge = cross_encoder_rank(prompt, row) or length_freq_rank(row)
...
st.write(
    "Enter a multiple-choice question and its five options. The model predicts the "
    "top-3 answers in ranked order using a three-tier retrieval lookup with a "
    "cross-encoder hedge. Built for the DL & GenAI course project (Roll No 23f3004491)."
)

EXAMPLE = {
    "prompt": "Pick the best possible answer: What is the primary source of the Sun's energy?",
...
prompt = st.text_area("Question", key="prompt", height=80)
col1, col2 = st.columns(2)
a = col1.text_input("Option A", key="A")
b = col2.text_input("Option B", key="B")
c = col1.text_input("Option C", key="C")
d = col2.text_input("Option D", key="D")
e = st.text_input("Option E", key="E")
...
if st.button("Predict Top-3", type="primary"):
    if not prompt.strip():
        ...
        with st.spinner("Scoring options..."):
            ranked, source = predict(prompt, opts)
        labels = dict(zip(OPTIONS, opts))
        st.success(f"Predicted ranking (top 3): {' '.join(ranked)}")
        for rank, opt in enumerate(ranked, 1):
            st.markdown(f"**{rank}. Option {opt}** — {labels[opt]}")
